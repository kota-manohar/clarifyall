# Media.io Gemini AI Photo Prompts Scraper

This script scrapes Gemini AI photo prompts from Media.io and generates SQL INSERT statements for the `prompt_categories` and `prompts` tables.

## URL
- **Source**: https://www.media.io/image-effects/gemini-ai-photo-prompt-copy.html
...
```bash
node scrape-mediaio-prompts.js
```

This will:
- Scrape prompts and categories from the Media.io page
- Save raw data to `output/mediaio-prompts-raw.json`
- Save categories to `output/mediaio-categories.json`
...
- `output/mediaio-prompts-raw.json` - Raw scraped data (categories with prompts, updated with local image URLs)
- `output/mediaio-categories.json` - Categories only
- `../logos/` - Downloaded example images (e.g., `beach-photo-golden-hour-breeze.png`)
- `sql-output/insert-prompt-categories.sql` - SQL INSERT for categories
- `sql-output/insert-prompts.sql` - SQL INSERT for prompts

## Notes

- The script extracts categories like "Beach Photo", "Birthday Photo", "Boy", "Girl", "Couple", etc.
...
4. Adjust the selectors in `scrape-mediaio-prompts.js` if needed
