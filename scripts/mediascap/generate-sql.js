...
function generateSQL() {
  ...
  if (!fs.existsSync(RAW_DATA_FILE)) {
    console.error(`❌ Error: ${RAW_DATA_FILE} not found. Please run scrape-mediaio-prompts.js first.`);
    process.exit(1);
  }
  ...
  let categoriesSQL = `-- ============================================\n`;
  categoriesSQL += `-- Insert Prompt Categories from Media.io\n`;
  ...
  let promptsSQL = `-- ============================================\n`;
  promptsSQL += `-- Insert Prompts from Media.io\n`;
  ...
  categories.forEach((category, catIndex) => {
    ...
    category.prompts.forEach((prompt, promptIndex) => {
      ...
      // Extract tags from prompt text (simple keyword extraction)
...
function extractTags(promptText) {
  ...
  // Extract style keywords from "Style Keywords:" section if present
