...
const LOGOS_DIR = path.join(__dirname, '..', 'logos'); // Shared logos directory
 ...
 */
function generateLogoFilename(toolName, originalUrl, convertSvg = true) {
...
/**
 * Download all logos for Futurepedia tools
 */
async function downloadFuturepediaLogos() {
    console.log('📥 Downloading Futurepedia logos...\n');
    ...
    for (let i = 0; i < tools.length; i++) {
        ...
        // Skip if logo_url is already a full URL pointing to clarifyall.com
        ...
        try {
            ...
            if (logoFilename.endsWith('.svg')) {
                ...
                const pngFilename = logoFilename.replace('.svg', '.png');
                const pngPath = path.join(LOGOS_DIR, pngFilename);
                ...
                try {
                    ...
                    // Update references to use PNG
                    tool.logo_url = `https://clarifyall.com/logos/${pngFilename}`;
                    logoMapping.set(tool.name, pngFilename);

                    const pngStats = fs.statSync(pngPath);
                    console.log(`  ✅ Converted to PNG: ${pngFilename} (${(pngStats.size / 1024).toFixed(2)} KB)`);
                } catch (conversionError) {
                    console.log(`  ⚠️  SVG conversion failed, keeping original: ${conversionError.message}`);
                    // Keep the SVG file if conversion fails
                    tool.logo_url = `https://clarifyall.com/logos/${logoFilename}`;
                    logoMapping.set(tool.name, logoFilename);
                }
            } else {
                // Update tool with full logo URL for non-SVG files
                tool.logo_url = `https://clarifyall.com/logos/${logoFilename}`;
                logoMapping.set(tool.name, logoFilename);
    ...
    return {
        ...
        stats: { success: downloaded, failed, total: tools.length }
    };
}

// Run if called directly
if (require.main === module) {
    downloadFuturepediaLogos()
        .then(() => {
...
module.exports = downloadFuturepediaLogos;
