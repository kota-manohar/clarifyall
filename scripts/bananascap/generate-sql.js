...
function generateSQL() {
  ...
  if (!fs.existsSync(RAW_DATA_FILE)) {
    console.error(`❌ Error: ${RAW_DATA_FILE} not found. Please run scrape-banana-prompts.js first.`);
    process.exit(1);
  }
  ...
  let categoriesSQL = `-- ============================================\n`;
  categoriesSQL += `-- Insert Prompt Categories from BananaPrompts.xyz\n`;
  ...
  const categoryValues = categories.map((cat, index) => {
    const name = cat.name.replace(/'/g, "''"); // Escape single quotes
    const description = (cat.description || '').replace(/'/g, "''");
    const icon = cat.icon || '✨';
    const orderIndex = cat.order_index !== undefined ? cat.order_index : index;
  ...
  let promptsSQL = `-- ============================================\n`;
  promptsSQL += `-- Insert Prompts from BananaPrompts.xyz\n`;
  ...
  categories.forEach((category, catIndex) => {
    ...
    category.prompts.forEach((prompt, promptIndex) => {
      ...
      const orderIndex = prompt.order_index || promptIndex;
      
      // Extract tags from prompt text and existing tags
      const tags = extractTags(promptText, prompt.tags || []);
      const tagsJSON = JSON.stringify(tags).replace(/'/g, "''");
      
      // Use category name as placeholder - will need to update with actual category_id
 ...
 * Extract tags from prompt text and existing tags
 */
function extractTags(promptText, existingTags = []) {
  ...
  const styleKeywords = [
    ...
    'ai generated', 'stable diffusion', 'midjourney', 'dalle', 'artificial intelligence'
  ];
