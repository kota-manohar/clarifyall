...
function generateInsertSQL() {
  ...
  const categorySql = [];
  categorySql.push('-- ============================================');
  categorySql.push('-- INSERT CATEGORIES FROM AITOOLSDIRECTORY.COM');
  categorySql.push('-- ============================================');
  categorySql.push('');
  ...
  validTools.forEach(({ tool, categoryName }) => {
    ...
    // Use description for short_description (limit to 500 chars as per schema)
    let shortDescription = tool.description || tool.full_description || 'AI tool from aitoolsdirectory.com';
    if (shortDescription.length > 500) {
      shortDescription = shortDescription.substring(0, 497) + '...';
    }

    // Map pricing_model to allowed ENUM values: 'FREE','FREEMIUM','FREE_TRIAL','PAID'
    let pricingModel = tool.pricing_model;
    if (pricingModel === 'OPEN_SOURCE') {
      pricingModel = 'FREE';
    } else if (!['FREE', 'FREEMIUM', 'FREE_TRIAL', 'PAID'].includes(pricingModel)) {
      pricingModel = 'FREE'; // Default fallback
    }
  ...
  function writeToolSqlFile(index, tools) {
    const sql = [];
    sql.push('-- ============================================');
    sql.push(`-- INSERT TOOLS FROM AITOOLSDIRECTORY.COM - PART ${index}`);
