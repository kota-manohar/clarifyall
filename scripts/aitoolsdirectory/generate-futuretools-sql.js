...
const CATEGORY_MAPPING_FILE = path.join(OUTPUT_DIR, 'futuretools-category-mapping.json');
...
/**
 * Generate SQL INSERT scripts for FutureTools
 */
function generateFutureToolsSQL() {
    console.log('📝 Generating SQL INSERT scripts for FutureTools...');

    if (!fs.existsSync(LOGOS_DATA_FILE)) {
        throw new Error(`Logos data file not found: ${LOGOS_DATA_FILE}. Please run download-futuretools-logos.js first.`);
    }

    const tools = fs.readJsonSync(LOGOS_DATA_FILE);
    ...
    categoryMapping.forEach(mapping => {
        categoryMap.set(mapping.futuretools_name, mapping);
    });
    ...
    // Split into multiple files (500 tools per file due to BLOB size)
    ...
    validTools.forEach(tool => {
        ...
        let shortDescription = tool.description || tool.full_description || 'AI tool from FutureTools.io';
        if (shortDescription.length > 500) {
            shortDescription = shortDescription.substring(0, 497) + '...';
        }

        const fullDescription = tool.full_description || tool.description || 'AI tool from FutureTools.io';
        ...
        // Prepare logo binary data
        const logoBinary = tool.logo_binary_base64 ? bufferToHex(tool.logo_binary_base64) : 'NULL';
        const logoMimeType = tool.logo_mime_type ? escapeSql(tool.logo_mime_type) : 'NULL';
        ...
        const toolValue = {
            ...
            og_image: tool.logo_url ? escapeSql(tool.logo_url) : 'NULL',
            twitter_title: twitterTitle,
            twitter_description: twitterDescription,
            canonical_url: 'NULL',
            structured_data: 'NULL',
            logo_url: tool.logo_url ? escapeSql(tool.logo_url) : 'NULL',
        ...
        if (currentFileTools.length >= toolsPerFile) {
            ...
            toolSqlFiles.push(`insert-tools-futuretools-part${fileIndex}.sql`);
            fileIndex++;
            currentFileTools = [];
        }
    });
    ...
    function writeToolSqlFile(index, tools) {
        const sql = [];
        sql.push('-- ============================================');
        sql.push(`-- INSERT TOOLS FROM FUTURETOOLS.IO - PART ${index}`);
  ...
  ${tool.rating}, ${tool.review_count}, ${tool.userId}, ${tool.logo_binary}, ${tool.logo_mime_type}
        ...
        const filename = path.join(SQL_OUTPUT_DIR, `insert-tools-futuretools-part${index}.sql`);
...
module.exports = generateFutureToolsSQL;
